...
Каждый пример покрывает все четыре ручки. Комментарии в коде — **английские и
короткие**: они называют ручку и параметры в принятой для языка нотации (JSDoc,
docstrings, Javadoc, KDoc, rustdoc, PHPDoc, YARD, Doxygen, POD, edoc, Haddock,
roxygen2, LDoc, comment-based help), а всё, что требует объяснения — правила
работы с refresh-токенами, защита от переигрывания, ограничения claims, коды
ответов, — живёт в этом README. Так страница переводится целиком, а код остаётся
одинаковым на всех языках сайта.
