...
- **`.unwrap()` распространён** в текущем коде (особенно в `key.rs`, `jwt.rs`) —
  панику на некорректных данных легко словить. Новый код пишите с корректной
  обработкой ошибок через `Result`/`?` и типы из `error.rs` / `models/jwt.rs`.
- **Обработка ошибок хранилища `jti` при выпуске неполная**: ошибка `store_jti`
  логируется, но не прерывает выпуск. Учитывайте это при изменениях.
...
- **С каждым коммитом обязательно поднимайте версию** в `Cargo.toml` (как минимум
  патч по semver). Учтите: пуш такого изменения в `master` запускает релиз и
  публикацию Docker-образов (см. «Соглашения и подводные камни»).
