...
| `JWKS_CACHE_STALE_MAX_SECONDS` | `3600` | Предельный возраст снимка JWKS, который отдаётся при недоступном сервисе ключей (stale-while-revalidate). Отсчитывается от момента получения снимка, поэтому значение не больше TTL отключает отдачу устаревших снимков; `0` — отключает явно. |
...
- **Кеш публичных ключей (`jwk.rs`).** JWKS кешируется в памяти: до этого каждая
  ...
  - **Недоступный JWKS не кладёт верификацию** (`JWKS_CACHE_STALE_MAX_SECONDS`):
    если обновление не удалось, отдаётся последний известный снимок, пока его
    возраст не превысил предел. Это размен доступности на актуальность, поэтому
    предел обязателен — без него отозванный ключ считался бы валидным вечно, —
    а каждая такая отдача пишется в лог (WARN) и в метрику
