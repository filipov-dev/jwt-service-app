...
> **Локальный clippy может отставать от CI.** CI ставит свежий stable, поэтому
> новые линты появляются там раньше, чем у вас: правка `manual_option_zip` в
> JWT-31 прошла локально на 1.94 и упала на 1.97. Если CI ругается на лint,
> которого локально нет, — сверьте `cargo clippy --version`, а не спорьте с
> линтом.
